...
*Your mileage may vary - that's why we use relative gates!*

## Troubleshooting

### "Performance test passed when it should fail"

The bug may not be impactful enough. Try:
- Increasing input size in test harness
- Lowering the performance gate threshold
- Injecting a more severe bug

### "Correctness test failed"

The bug broke functionality. Check:
...
Performance tests can be affected by system load. Try:
- Increase warmup runs
- Increase measurement runs (higher n_runs)
- Run on a quieter machine
- Use relative gates (we already do this)

## Contributing

Found a good performance bug pattern? PRs welcome!

1. Fork the repo
2. Create task on `perf-bugs-suite` branch
3. Follow the development workflow above
4. Submit PR with validation passing

## License

Same as the original keon/algorithms repository (MIT).

## Citation

If you use this suite in research, please cite:

```bibtex
@misc{keon_algorithms_perf_suite,
  title={Performance Bug Test Suite for SWE-Agent},
  author={Based on keon/algorithms by keon},
  year={2025},
  url={https://github.com/YOUR_USERNAME/algorithms/tree/perf-bugs-suite}
}
```
...
- Performance testing methodology inspired by SWE-bench
- Built for [SWE-Agent](https://github.com/princeton-nlp/SWE-agent) evaluation
